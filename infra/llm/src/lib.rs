...
impl LlmProvider for OpenAiCompatibleAdapter {
    async fn generate_fix(&self, prompt: &FixPrompt) -> Result<FixProposal, LlmError> {
        let system_prompt = "You are yunq's automated Remediation Agent. Your job is to fix code analysis findings accurately.\n\
        ...
        let parsed: FixProposalJson = serde_json::from_str(clean_json)
            .map_err(|e| LlmError::InvalidOutput(format!("JSON fix proposal schema mismatch: {e}. Raw content: {content}")))?;
