...
impl GitLabAlmAdapter {
    ...
    pub async fn post_mr_comment(&self, mr_id: u64, comment: &str) -> Result<(), AlmError> {
        let url = format!("{}/api/v4/projects/{}/merge_requests/{}/notes", self.base_url, self.project_id, mr_id);
        let resp = self
            .client
            .post(&url)
...
impl AlmStatusReporter for GitLabAlmAdapter {
    async fn report_commit_status(&self, sha: &CommitSha, status: &CommitStatus) -> Result<(), AlmError> {
        ...
        let url = format!("{}/api/v4/projects/{}/statuses/{}", self.base_url, self.project_id, sha.as_str());
...
pub fn generate_mr_decoration_comment(report: &AnalysisReport) -> String {
    ...
    comment.push_str(&format!("* **Security Hotspots**: {}\n\n", report.hotspots().len()));
