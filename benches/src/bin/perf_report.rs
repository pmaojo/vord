...
use yunq_benchmarks::{is_regression, percentile, PerfReport};
...
fn main() {
    ...
    if let Some(pos) = args.iter().position(|a| a == "--compare") {
        let baseline_path = args.get(pos + 1).expect("--compare needs <baseline.json> <current.json>");
        let current_path = args.get(pos + 2).expect("--compare needs <baseline.json> <current.json>");
...
fn run_benchmark() -> PerfReport {
    ...
    assert!(!files.is_empty(), "corpus at {corpus:?} is empty — check benches/corpus/rust/");
...
fn compare(baseline_path: &str, current_path: &str) {
    ...
    println!("baseline throughput: {:.1} LOC/s", baseline.throughput_loc_per_sec);
    println!("current throughput:  {:.1} LOC/s", current.throughput_loc_per_sec);
    println!("baseline p50/p99 file scan: {:.2}ms / {:.2}ms", baseline.p50_file_scan_ms, baseline.p99_file_scan_ms);
    println!("current  p50/p99 file scan: {:.2}ms / {:.2}ms", current.p50_file_scan_ms, current.p99_file_scan_ms);
    ...
    if is_regression(baseline.throughput_loc_per_sec, current.throughput_loc_per_sec) {
        let drop_pct = 100.0 * (1.0 - current.throughput_loc_per_sec / baseline.throughput_loc_per_sec);
        eprintln!("REGRESSION: throughput dropped {drop_pct:.1}% versus baseline (> 10% threshold)");
        std::process::exit(1);
    }
...
fn load_report(path: &str) -> PerfReport {
    ...
    serde_json::from_str(&contents).unwrap_or_else(|e| panic!("failed to parse {path} as a PerfReport: {e}"))
}
