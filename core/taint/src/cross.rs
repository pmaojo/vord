    ...
    pub fn find_flows(&self, files: &[(&str, &AstNode)]) -> Vec<CrossFileFlow> {
        ...
        for _ in 0..10 {
            ...
            for function in &functions {
                ...
                if let Some(existing) = summaries.get_mut(&function.key) {
                    if *existing != summary {
                        *existing = summary;
                        changed = true;
                    }
...
fn build_global_fallback(functions: &[FunctionInfo]) -> HashMap<String, FunctionKey> {
    ...
    for function in functions {
        if !map.contains_key(&function.key.1) {
            map.insert(function.key.1.clone(), function.key.clone());
        }
    }
    map
}
