...
/// One function's cyclomatic complexity, tagged with the file it came from.
/// Computed once per parsed file inside `AnalyzerService::analyze_one`
/// (`crate::function_complexity::compute`) and threaded onto the report the
/// same way `Issue`s are, so a CRAP-style risk score can be computed later
/// by joining this against [`FileCoverage`] without a second parse — see
...
impl AnalysisReport {
    ...
    pub fn set_duplications(&mut self, duplication: DuplicationReport) {
        self.metrics
            .set_duplication(duplication.duplicated_lines, duplication.clone_sets.len());
    ...
    /// Stores every function's cyclomatic complexity, computed once per
...
const MEASURE_TABLE: &[(&str, MeasureFn)] = &[
    ...
    ("mutants_killed", |r| {
        r.mutation.map(|m| m.killed_mutants as f64)
    }),
    ("mutants_survived", |r| {
        r.mutation.map(|m| m.survived_mutants as f64)
    }),
    ("mutants_timeout", |r| {
        r.mutation.map(|m| m.timeout_mutants as f64)
    }),
    ("mutants_no_coverage", |r| {
        r.mutation.map(|m| m.no_coverage_mutants as f64)
    }),
    ("mutation_score", |r| {
        r.mutation.and_then(|m| m.mutation_score())
    }),
...
mod tests {
    ...
    fn crap_joins_complexity_and_coverage_into_findings_and_measures() {
        ...
        let findings = report
            .compute_crap_findings()
            .expect("coverage report present");
        assert_eq!(
            findings.len(),
            1,
            "the low-complexity fully-covered function stays silent"
        );
    ...
    fn set_duplications_counts_each_duplicated_line_once() {
        ...
        report.set_duplications(DuplicationReport {
            clone_sets: vec![set],
            duplicated_lines: 40,
        });
    ...
    fn external_issues_fold_into_severity_counts_debt_and_the_security_rating() {
        ...
        report.add_external_issues(vec![
            ...
            ExternalIssue::new(
                issue("codeql:js-sql-injection", Severity::Blocker),
                IssueType::Vulnerability,
            ),
        ]);
        ...
        assert_eq!(
            ...
            Rating::from_severity(Severity::Blocker)
        );
    ...
    fn an_external_issue_with_an_effort_estimate_adds_debt_and_per_rule_effort() {
        ...
        assert_eq!(
            report.remediation_effort().by_component.get("hash.go"),
            Some(&15)
        );
    }
}
