//! `// yunq-ignore` suppression comments: an escape hatch for the rare line
...
//!
//! `// yunq-ignore` alone (no colon) suppresses every rule on that line.
//! `// yunq-ignore: rule-id[, rule-id...]` suppresses only the listed
...
mod tests {
    ...
    fn bare_ignore_suppresses_any_rule() {
        let content = "let x = 1; // yunq-ignore\n";
    ...
    fn scoped_ignore_only_suppresses_listed_rules() {
        let content = "let x = 1; // yunq-ignore: secrets:high-entropy-string\n";
        assert!(is_suppressed(content, 1, "secrets:high-entropy-string"));
        assert!(!is_suppressed(content, 1, "smells:unwrap-usage"));
    }
    ...
    fn scoped_ignore_accepts_a_comma_separated_list() {
        let content = "let x = 1; // yunq-ignore: rule-a, rule-b\n";
