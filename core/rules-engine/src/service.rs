    ...
    M: MetricsTracker,
{
        ...
        let tokenized: Vec<vord_cpd::TokenizedFile> = files
            ...
            .map(|file| {
                let mut tokenized = self
                    ...
                    .get(file.language())
                    .map(|parser| {
                        parser.tokenize_for_duplication(file, self.duplication.normalization)
                    })
                    .unwrap_or_else(|| vord_cpd::TokenizedSource {
                ...
                if !self.duplication.include_test_code {
                    ...
                    tokenized
                        .lines
                        .retain(|(line, _)| !crate::in_ranges(&test_ranges, *line));
                    tokenized
                        .declaration_lines
                        .retain(|line| !crate::in_ranges(&test_ranges, *line));
                }
                vord_cpd::TokenizedFile {
    ...
    fn analyze_one(&self, file: &SourceFile, config_hash: u64) -> FileOutcome {
        ...
        let function_complexities: Vec<FileFunctionComplexity> =
            crate::function_complexity::compute(&ast)
                .into_iter()
