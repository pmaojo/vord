...
use yunq_agent_policy::{AgentPolicy, Evaluation, Finding};
...
pub fn denial_text(path: &str, evaluation: &Evaluation, timing: Timing) -> String {
...
pub fn claude_code_output(event: &str, verdict: &Verdict) -> Option<serde_json::Value> {
    match (event, verdict) {
        ...
        ("PreToolUse", Verdict::Deny { path, evaluation }) => Some(serde_json::json!({
            "hookSpecificOutput": {
                ...
                "permissionDecisionReason": denial_text(path, evaluation, Timing::Prevented),
            }
        })),
        ...
        ("PostToolUse", Verdict::Deny { path, evaluation }) => Some(serde_json::json!({
            "decision": "block",
            "reason": denial_text(path, evaluation, Timing::AlreadyWritten),
        })),
...
pub async fn run_claude_code() -> anyhow::Result<std::process::ExitCode> {
    ...
    if let Some(output) = claude_code_output(&payload.hook_event_name, &verdict) {
...
pub async fn run_check(file: PathBuf) -> anyhow::Result<std::process::ExitCode> {
    ...
    match verdict {
        ...
        Verdict::Advise { path, evaluation } => {
            eprintln!("{}", advisory_text(&path, &evaluation));
            Ok(std::process::ExitCode::SUCCESS)
        }
        Verdict::Deny { path, evaluation } => {
            ...
            eprintln!("{}", denial_text(&path, &evaluation, Timing::AlreadyWritten));
...
mod tests {
    ...
    fn pre_tool_use_emits_a_deny_decision() {
        ...
        let output = claude_code_output("PreToolUse", &verdict).expect("emits");
    ...
    fn post_tool_use_blocks_with_a_reason_instead_of_a_permission_decision() {
        ...
        let verdict = Verdict::from_evaluation("a.py".to_string(), evaluation);
        let output = claude_code_output("PostToolUse", &verdict).expect("emits");
    ...
    fn the_two_events_disagree_about_whether_the_file_was_written() {
        ...
        let prevented = denial_text("a.py", &evaluation, Timing::Prevented);
        assert!(prevented.contains("was NOT written"), "{prevented}");

        let landed = denial_text("a.py", &evaluation, Timing::AlreadyWritten);
    ...
    fn a_silent_verdict_emits_nothing_on_either_event() {
        assert!(claude_code_output("PreToolUse", &Verdict::Silent).is_none());
        assert!(claude_code_output("PostToolUse", &Verdict::Silent).is_none());
    }
    ...
    fn pre_tool_use_never_emits_allow_for_an_advisory() {
        ...
        assert!(claude_code_output("PreToolUse", &verdict).is_none());
    }
