...
impl Backend {
    async fn analyze_and_publish(&self, uri: Url, text: String) {
        ...
        self.documents.lock().expect("document map lock poisoned").insert(uri.clone(), text);
...
impl LanguageServer for Backend {
    async fn initialize(&self, _: InitializeParams) -> RpcResult<InitializeResult> {
        Ok(InitializeResult {
            server_info: Some(ServerInfo { name: "yunq-lsp".into(), version: Some(env!("CARGO_PKG_VERSION").into()) }),
    ...
    async fn initialized(&self, _: InitializedParams) {
        self.client.log_message(MessageType::INFO, "yunq-lsp ready").await;
    }
    ...
    async fn did_close(&self, params: DidCloseTextDocumentParams) {
        ...
        self.documents.lock().expect("document map lock poisoned").remove(&uri);
...
async fn main() {
    ...
    let (service, socket) =
        LspService::new(|client| Backend { client, documents: Mutex::new(HashMap::new()) });
