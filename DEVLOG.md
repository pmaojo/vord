# yunq Development Log — phases 1–7 as they were built

> **This file is the archive, not the plan.** It was `ROADMAP.md` until the
> 2026-07-29 reform; the forward-looking plan now lives in
> [ROADMAP.md](ROADMAP.md). Everything below is kept verbatim because the
> design rationale it records — why a choice was made, and what the naive
> alternative got wrong — is the expensive part and is not reproduced in the
> new roadmap.
>
> Read it for *why* something is the way it is. Do not read it for *what is
> true today*: two classes of entry are known stale.
>
> - References to `bin/server`, `bin/worker`, `bin/frontend` and
>   `infra/postgres` describe work done before those components were split
>   into the private `yunq-cloud` repo — they no longer exist here.
> - The enterprise checklist near the end still cites "~398k LOC/s". That
>   figure was retracted by the performance-pillar entry in this same file:
>   the measured floor is **~67.6k LOC/s**. The retraction is correct; the
...
pass through silently. Surfaced, not fixed, in this pass — closing it means
walking `scoped_identifier` expressions generally, not only ones rooted in
a `use_declaration`, deduped per `(from, to)` pair so one file's repeated
references to the same external crate don't produce a finding per
reference.
