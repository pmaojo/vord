...
use crate::common::{declared_methods, field_mutations, field_names, is_domain_path, is_value_object};
...
impl ValueObjectMutationRule {
    pub fn new() -> Self {
        Self { id: RuleId::new("ddd:value-object-mutation").expect("valid rule id") }
    }
}
...
mod tests {
    ...
    fn check(path: &str, code: &str, language: LanguageIdentifier) -> Vec<Finding> {
        ...
        ValueObjectMutationRule::new().check(&[(file, ast)]).into_iter().map(|(_, f)| f).collect()
    }
    ...
    fn flags_a_typescript_value_object_that_mutates_in_place() {
        ...
        let findings = check("src/domain/money.ts", code, LanguageIdentifier::typescript());
        assert_eq!(findings.len(), 1, "{findings:?}");
        assert!(findings[0].message.contains("`Money::add`"), "{}", findings[0].message);
    }

    #[test]
    fn silent_on_the_constructor_itself() {
        let code = "export class Money {\n  private amount: number;\n  constructor(amount: number) {\n    this.amount = amount;\n  }\n}\n";
        assert!(check("src/domain/money.ts", code, LanguageIdentifier::typescript()).is_empty());
    }
    ...
    fn silent_outside_the_domain_layer() {
        let code = "export class Money {\n  private amount: number;\n  constructor(amount: number) {\n    this.amount = amount;\n  }\n  add(other: Money): void {\n    this.amount = this.amount + other.amount;\n  }\n}\n";
        assert!(check("src/adapters/http/money.ts", code, LanguageIdentifier::typescript()).is_empty());
    }
    ...
    fn a_class_with_no_fields_is_not_claimed_to_be_a_value_object() {
        ...
        assert!(check("src/domain/placeholder.ts", code, LanguageIdentifier::typescript()).is_empty());
    }
}
