...
fn collection_fields(class: &ClassInfo<'_>) -> BTreeSet<String> {
    let mut fields: BTreeSet<String> = class
        ...
        .filter(|field| {
            field
                .declared_type
                .as_deref()
                .is_some_and(is_collection_type)
        })
    ...
    for constructor in class.methods.iter().filter(|m| is_constructor(m, class)) {
            ...
            .filter(|n| *n.kind() == NodeKind::Assignment)
        {
            ...
            let Some(base) = target.first_child() else {
                continue;
            };
            let Some(name) = target.children().get(1) else {
                continue;
            };
            ...
            if COLLECTION_INITIALIZERS
                .iter()
                .any(|init| value_text.starts_with(init))
            {
...
mod tests {
    ...
    fn flags_a_typescript_getter_returning_the_internal_array() {
        ...
        assert!(
            findings[0].message.contains("`Order::getItems`"),
            "{}",
            findings[0].message
        );
