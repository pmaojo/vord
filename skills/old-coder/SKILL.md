...
> Vendored from [AmazingAng/old-coder](https://github.com/AmazingAng/old-coder)
> (MIT license, copyright notice preserved in this directory's `LICENSE`).
> Unmodified apart from the "Using this with vord" section appended at the
> end, which wires the GAUNTLET's static-analysis layer to `vord scan`.
...
## Using this with vord

If `vord` is on `PATH` (see the repository root `SKILL.md`), it can serve
the GAUNTLET's static-analysis-shaped layers directly instead of, or
alongside, per-ecosystem linters:

- **Lint + format / static types (partial)**: `vord scan --enforce-gate`
  fails closed (nonzero exit) on any blocker/critical finding, in any of
  vord's 24 supported languages, in under 30ms — cite the command and its
  exit code in EVIDENCE like any other layer. It complements rather than
  replaces `tsc`/`mypy`/etc.: vord's AST rules catch architecture,
  complexity, duplication, and security-pattern issues; a language's own
  type checker still owns type-soundness.
  Real per-tool lint coverage (not just parse errors) needs the tool run
  directly with `--sarif` merged in — see the root `SKILL.md`'s
  "Full-Coverage Linting" section.
- **Supply chain & secrets**: `vord scan` includes OWASP/secret-pattern
  rules as part of the same run; still pair with `pip-audit`/`npm
  audit`/`cargo-audit`/etc. for dependency-vulnerability data vord doesn't
  compute itself.
- **Checkpoint commits + worktree isolation**: if this task is running
  inside a `vord swarm` role worktree, the GAUNTLET's "commit at SPEC and
  at each GREEN/REFACTOR checkpoint" guidance and the swarm's own
  policy-scoped handoffs are the same discipline applied at two
  granularities — keep both; the swarm handoff is not a substitute for the
  EVIDENCE report this skill requires.

None of this changes the loop, the anti-gaming rules, or what belongs in
EVIDENCE — it only names which command satisfies which layer when vord is
available. Where vord doesn't reach (mutation testing, property-based
tests, coverage, real execution), keep using this file's default guidance
unchanged.
