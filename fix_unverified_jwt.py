def fix():
    with open('rulesets/owasp/src/unverified_jwt.rs', 'r') as f:
        content = f.read()

    new_content = """use yunq_ast::{AstNode, LanguageIdentifier, NodeKind, SourceFile};
...
impl Rule for UnverifiedJwtRule {
    ...
    fn check(&self, file: &SourceFile, ast: &AstNode) -> Vec<Finding> {
        ...
        for call in ast.descendants().filter(|n| *n.kind() == NodeKind::Call) {
            ...
            let call_text = call.text().replace(" ", "").replace("\\n", "").replace("\\r", "");
...
mod tests {
    ...
    fn check(code: &str, lang: LanguageIdentifier) -> Vec<Finding> {
        let file = SourceFile::new("app.test", code, lang).unwrap();
...
"""
    ...
    with open('rulesets/owasp/src/unverified_jwt.rs', 'w') as f:
        f.write(new_content)
